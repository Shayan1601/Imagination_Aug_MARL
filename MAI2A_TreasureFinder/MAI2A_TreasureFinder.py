...
if __name__ == "__main__":
    ...
    target_network1 = I2A_FindTreasure(input_size, output_size, hyperparameters_agent1.rollout_len)
    target_network2 = I2A_FindTreasure(input_size, output_size, hyperparameters_agent2.rollout_len)
    ...
    for episode in range(hyperparameters_agent1.num_episodes):
        ...
        for t in count():
            ...
            action_agent1 = select_action(model_agent1, state1, output_size,epsilon)
            action_agent2 = select_action(model_agent2, state2, output_size,epsilon)
            ...
            if len(memory_agent1) >= hyperparameters_agent1.batch_size:
                ...
                action_probs_agent1= model_agent1(states1, actions1)
                actions1 = actions1.unsqueeze(-1)
                      
                action_values_agent1 = torch.gather(action_probs_agent1, 1,actions1)
                               
                next_q_values1 = target_network1(next_states1, actions1).max(1)[0].detach()
                ...
                action_probs_agent2 = model_agent2(states2, actions2)
                actions2 = actions2.unsqueeze(-1)
                                    
                action_values_agent2 = torch.gather(action_probs_agent2, 1, actions2)
                
                next_q_values2 = target_network2(next_states2, actions2).max(1)[0].detach()
